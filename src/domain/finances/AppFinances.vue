...
const transactions = ref<Transaction[]>([
  {
    id: 1,
    trackingNumber: "M241128-0002",
    prn: "2250000039829",
    service: "SPECIAL LICENSE",
    amount: 50000,
    status: "SUCCESS",
    date: "2024-11-28T10:17:00",
  },
  {
    id: 2,
    trackingNumber: "M241127-0008",
    prn: "2250000039915",
    service: "MARRIAGE",
    amount: 50000,
    status: "FAILED",
    date: "2024-11-28T10:09:00",
  },
  {
    id: 3,
    trackingNumber: "M241126-0005",
    prn: "2250000039921",
    service: "LICENSE RENEWAL",
    amount: 100000,
    status: "PENDING",
    date: "2024-11-27T14:25:00",
  },
]);
  ...
  return transactions.value.filter((t) => {
    return (
      (!filters.value.search ||
        t.trackingNumber.includes(filters.value.search)) &&
      (!filters.value.status || t.status === filters.value.status) &&
      (!filters.value.startDate ||
        moment(t.date).isSameOrAfter(filters.value.startDate)) &&
      (!filters.value.endDate ||
        moment(t.date).isSameOrBefore(filters.value.endDate))
    );
  });
}
  ...
  <div class="w-full shadow-lg bg-white rounded-lg p-6">
    <div class="flex justify-between items-center py-3">
      <div class="text-primary-700 flex items-center">
        <i
          class="bg-primary-100 border border-primary-200 p-2 rounded-full fa-solid fa-money-bill"
        ></i>
      ...
      <div class="flex items-center">
        <button
          class="button btn-sm ml-96 pl-96 px-4 py-2 rounded-md text-white bg-secondary-500 hover:bg-secondary-600 transition-colors"
          @click="openAddTransactionModal = true"
        >
          <i class="fa-solid fa-plus-circle mr-2"></i> Add Transaction
        </button>
      </div>
    ...
    <div class="flex justify-between mb-6">
      <div class="grid grid-cols-4 gap-4 w-full max-w-xl">
        <input
          class="filter-element e-input w-full py-2 px-4 rounded-md border border-gray-300"
          type="text"
          placeholder="Search by Tracking Number"
          v-model="filters.search"
        />
        <select
          class="filter-element e-select w-full py-2 px-4 rounded-md border border-gray-300"
      ...
      <div class="flex items-center">
        <button
          class="button btn-sm w-auto px-4 py-2 rounded-md text-white bg-secondary-500 hover:bg-secondary-600 transition-colors"
          @click="clearFilters"
        >
      ...
      <table class="min-w-full table-auto border-separate border-spacing-0">
        <thead>
          <tr class="bg-gray-100">
            ...
            <th class="t-header text-left py-3 px-4 border-b">
              Tracking Number
            </th>
            <th class="t-header text-left py-3 px-4 border-b">
        ...
        <tbody>
          ...
          >
            ...
            <td class="py-2 px-4">
              <span class="hover:underline text-blue-600 cursor-pointer">{{
            ...
            <td class="py-2 px-4 text-center">
              <i
                class="fa-solid fa-eye p-2 mx-1 text-blue-600 bg-blue-100 border border-blue-200 hover:text-blue-700 rounded cursor-pointer"
                @click="viewDetails(transaction)"
              ></i>
              <i
                class="fa-solid fa-copy p-2 mx-1 text-gray-600 bg-gray-50 border border-gray-200 hover:text-primary-700 rounded cursor-pointer"
                @click="copy(transaction.trackingNumber)"
              ></i>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="page > 1" class="flex justify-between items-center mt-6">
      <button
        class="pagination-button px-4 py-2 text-white bg-primary-600 hover:bg-primary-700 rounded-md transition-colors"
        @click="previous"
        :disabled="page === 1"
      >
        <i class="fa-solid fa-arrow-left"></i> Previous
      </button>
      <label class="text-white bg-primary-700 px-3 py-2 rounded-md">{{
        page
      }}</label>
      <button
        class="pagination-button px-4 py-2 text-white bg-primary-600 hover:bg-primary-700 rounded-md transition-colors"
